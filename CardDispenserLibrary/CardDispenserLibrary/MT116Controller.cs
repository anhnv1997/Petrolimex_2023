...
{
    public class MT116Controller : ICardDispenser
    {
        public delegate void OnEvent(object? sender, MT116EventArgs e);

        public class MT116EventArgs
        {
            public int LenCard { get; set; } = 0;
            public string CardNumber { get; set; } = string.Empty;
            public string InputDetail { get; set; } = string.Empty;
            public int InputPort { get; set; } = 0;
            public bool isIn5Actived { get; set; } = false;
            public bool isIn6Actived { get; set; } = false;
            public bool isLoop1Actived { get; set; } = false;
            public bool isLoop2Actived { get; set; } = false;
            public bool isLoop3Actived { get; set; } = false;
            public bool isLoop4Actived { get; set; } = false;
            public string COM { get; set; } = string.Empty;
            public string StateCardDispenserCom1 { get; set; } = string.Empty;
            public string StateCardDispenserCom2 { get; set; } = string.Empty;
        ...
        public bool SetRelayDelayTime(int relayIndex, int timeInMiliscond)
        {
            string comport = this.ControllerInfor.Comport;
            int baudrate = this.ControllerInfor.Baudrate;
            string setNetworkInfor = CMD.SetRelayDelayTimeCMD(relayIndex, timeInMiliscond);
            this.IsBusy = true;
            string response = UdpTools.ExecuteCommand(comport, baudrate, setNetworkInfor, 500, Encoding.ASCII);
            this.IsBusy = false;
            //Char(2) + ChangeIP?/OK + char(3)	
        ...
        public int GetModeAutoCollectCard()
        {
            string comport = this.ControllerInfor.Comport;
            int baudrate = this.ControllerInfor.Baudrate;
            string autoDetectCMD = CMD.GetModeAutoCollectCardCMD;
            this.IsBusy = true;
            string response = UdpTools.ExecuteCommand(comport, baudrate, autoDetectCMD, 500, Encoding.ASCII);
            this.IsBusy = false;
            //Char(2)  + version  + “/”  + IPAdress + “/”  Port + “/” + subNetMask + “/”  + DefaultGateway + “/”+ MacAddress + char(3)
            if (response != null && response.Length > 0)
            {
                string[] data = response.Split('/');
                Dictionary<string, string> map = iPGS.Tools.EventHelpers.GetEventContent(data);
                string modeAutoCollectCard = map.ContainsKey("ModeAutoCollectCard".ToLower()) ? map["ModeAutoCollectCard".ToLower()] : "";

                int ReaderIndex = Regex.IsMatch(modeAutoCollectCard, @"^\d+$") ? Convert.ToInt32(modeAutoCollectCard) : -1;
                return ReaderIndex;
            }
            if (ErrorEvent != null)
            {
        ...
        public int GetTimeOutAutoCollectCard()
        {
            string comport = this.ControllerInfor.Comport;
            int baudrate = this.ControllerInfor.Baudrate;
            string autoDetectCMD = CMD.GetTimeOutAutoCollectCardCMD;
            this.IsBusy = true;
            string response = UdpTools.ExecuteCommand(comport, baudrate, autoDetectCMD, 500, Encoding.ASCII);
            this.IsBusy = false;
            //Char(2)  + version  + “/”  + IPAdress + “/”  Port + “/” + subNetMask + “/”  + DefaultGateway + “/”+ MacAddress + char(3)
            if (response != null && response.Length > 0)
            {
                string[] data = response.Split('/');
                Dictionary<string, string> map = iPGS.Tools.EventHelpers.GetEventContent(data);
                string timeOutAutoCollectCard = map.ContainsKey("TimeOutAutoCollectCard".ToLower()) ? map["TimeOutAutoCollectCard".ToLower()] : "";

                int timeOutAutoCollectCard_int = Regex.IsMatch(timeOutAutoCollectCard, @"^\d+$") ? Convert.ToInt32(timeOutAutoCollectCard) : -1;
                return timeOutAutoCollectCard_int;
            }
            if (ErrorEvent != null)
            {
                ErrorEvent(this, new ErrorEventArgs()
        ...
        public bool SetTimeOutAutoCollectCard(int timeInSecond)
        {
            string comport = this.ControllerInfor.Comport;
            int baudrate = this.ControllerInfor.Baudrate;
            string setNetworkInfor = CMD.SetTimeOutAutoCollectCardCMD(timeInSecond);
            this.IsBusy = true;
            string response = UdpTools.ExecuteCommand(comport, baudrate, setNetworkInfor, 500, Encoding.ASCII);
            this.IsBusy = false;
            //Char(2) + ChangeIP?/OK + char(3)	
        ...
        public int GetModeButton()
        {
            string comport = this.ControllerInfor.Comport;
            int baudrate = this.ControllerInfor.Baudrate;
            string autoDetectCMD = CMD.GetModeButtonCMD;
            this.IsBusy = true;
            string response = UdpTools.ExecuteCommand(comport, baudrate, autoDetectCMD, 500, Encoding.ASCII);
            this.IsBusy = false;
            //Char(2)  + version  + “/”  + IPAdress + “/”  Port + “/” + subNetMask + “/”  + DefaultGateway + “/”+ MacAddress + char(3)
            if (response != null && response.Length > 0)
            {
                string[] data = response.Split('/');
                Dictionary<string, string> map = iPGS.Tools.EventHelpers.GetEventContent(data);
                string timeOutAutoCollectCard = map.ContainsKey("ModeButton".ToLower()) ? map["ModeButton".ToLower()] : "";
