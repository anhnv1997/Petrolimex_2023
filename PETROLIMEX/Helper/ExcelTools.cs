    ...
    public class ExcelTools
    {
        ...
        public static void CreateTableHeader(SLDocument sl, DataGridView dgvData, string tittle)
        {
            ...
            foreach (DataGridViewColumn column in dgvData.Columns)
            {
                ...
                var textSize = TextFormatingTool.GetTextSize(column.HeaderText, dgvData.ColumnHeadersDefaultCellStyle.Font);
                var columnWidth = Math.Truncate(textSize.Width / 7d * 256) / 256;
                sl.SetColumnWidth(columnIndex, columnWidth);
                currentChar = TextFormatingTool.NextCharacter(currentChar);
            ...
            foreach (DataGridViewRow row in dgvCard.Rows)
            {
                foreach (DataGridViewCell cell in row.Cells)
                {
                    ...
                    if (cell.Value != null)
                    {
                        ...
                        var textSize = TextFormatingTool.GetTextSize(cell.Value == null ? "" : cell.Value.ToString() ?? "", dgvCard.ColumnHeadersDefaultCellStyle.Font);
        ...
        public static void SaveReportFile(SLDocument sl, string fileName)
        {
            ...
            {
                LogHelperv2.Logger_CONTROLLER_Error($"Exception SaveReportFile ex = {ex}", LogHelperv2.SaveLogFolder);
            }
        }
