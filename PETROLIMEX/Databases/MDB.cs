...
namespace IPGS.Object.Databases
{
    public class Mdb
    {
        ...
        // MS SQL SERVER
        private readonly string _sqlServerName = "103.127.207.247";
        private readonly string _sqlDatabaseName = "PETROLIMEX";
        private readonly string _sqlAuthentication = "Windows Authentication";
        private readonly string _sqlUserName = "kienpt";
        private readonly string _sqlPassword = "Kztek@123456";
        ...
        private bool OpenMdb()
        {
            ...
            else
            {
                ...
                {
                    LogHelperv2.Logger_CONTROLLER_Error($"Exception OpenMdb ex = {ex}", LogHelperv2.SaveLogFolder);
                }
            }
            return false;
        }
        ...
        public void CloseMdb()
        {
            ...
            {
                LogHelperv2.Logger_CONTROLLER_Error($"Exception CloseMdb ex = {ex}", LogHelperv2.SaveLogFolder);
            }
        }
        ...
        public bool ExecuteCommand(string commandString)
        {
            ...
            else
            {
                if (StateMDB())
                {
                    ...
                    {
                        LogHelperv2.Logger_CONTROLLER_Error($"Exception StateMDB ex = {ex}", LogHelperv2.SaveLogFolder);
                    }
                }
                else if (OpenMdb())
                {
                    ...
                    {
                        LogHelperv2.Logger_CONTROLLER_Error($"Exception OpenMdb2 ex = {ex}", LogHelperv2.SaveLogFolder);
                    }
                }
            }
            return false;
        ...
        public bool ExecuteCommand(string commandString, string parameters, byte[]? values)
        {
            if (!_isMdb)
            {
                if (State())
                {
                    ...
                    {
                        LogHelperv2.Logger_CONTROLLER_Error($"Exception State ex = {ex}", LogHelperv2.SaveLogFolder);
                    }
                }
                else if (OpenMdb())
                {
                    ...
                    {
                        LogHelperv2.Logger_CONTROLLER_Error($"Exception OpenMdb3 ex = {ex}", LogHelperv2.SaveLogFolder);
                    }
                }
            }
            else
